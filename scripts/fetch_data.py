...
class FetchAndLoad():
    ...
    def __init__(self, polygon:Polygon, epsg:int=4326, state='IA_FullState'):
        ...
        self.pipe_path = pipeline_path
        self.las_path = self.state+'.las'
        self.tif_path = self.state+'.tif'
        self.api_path = data_path +self.state+"/ept.json"
    ...
    def prepare_pipe(self):
        ...
        data = self.file_helper.read_json(pipeline_path)
    ...
    def read_laz(self):
        ...
        try:
            ...
            las = laspy.file.File(self.las_path , mode='r')
            self.las_file = las
            return las
